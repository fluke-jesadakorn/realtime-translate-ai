...
(function(){
  ...
  function rebuildFeeds(){
    ...
    state.history.forEach(e => renderEntry(e, false));
  }
  ...
  // Scroll synchronization between source and translation feeds
  const srcFeed = $("src-feed");
  const tgtFeed = $("tgt-feed");
  let isScrollingSrc = false;
  let isScrollingTgt = false;
  srcFeed.addEventListener("scroll", () => {
    if (isScrollingTgt) { isScrollingTgt = false; return; }
    isScrollingSrc = true;
    tgtFeed.scrollTop = srcFeed.scrollTop;
  });
  tgtFeed.addEventListener("scroll", () => {
    if (isScrollingSrc) { isScrollingSrc = false; return; }
    isScrollingTgt = true;
    srcFeed.scrollTop = tgtFeed.scrollTop;
  });
})();
</script>
</body>
